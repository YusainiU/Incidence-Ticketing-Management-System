@php
    ...
    $alertErr = Config::get('steps.alert.danger');
    $alertInf = Config::get('steps.alert.info');
    $alertWrn = Config::get('steps.alert.warning');
    $errorMsg = "text-red-600 $text";
    $fieldClass = 'flex flex-col pb-3';
    $labelClass = 'md:text-lg text-gray-500 dark:text-gray-400';
...
<div class="mb-10">
    <form wire:submit='createNewTicket'>
        <x-page-profile>
            <x-slot name="title">
                <span class="block text-2xl">Create New Ticket</span>
            ...
            <x-slot name="nav">
                <div class="flex flex-col gap-2 w-full justify-center">
                    ...
                    @if (count($ferrors))
                        <div class="{{ $alertErr }} font-bold text-center">
                            <h4 class="mb-2">Errors ..</h4>
                            {{ implode(' | ', $ferrors) }}
                        </div>
                    @endif
                    <div class="{{ $alertInf }} font-bold text-center">
                        Please make sure that there is asset and service level agreement associated with this site. In order to be able to select contact, please add contact to this site.
                    </div>
                </div>
            </x-slot>
            <x-slot name="left_panel">
                ...
                <div x-data="{
                    ...
                    <div class="p-3 rounded-lg flex gap-3 w-full shadow-lg x-50 absolute flex-col bg-white mt-3 z-50"
                        ...
                        <input x-model="filter" placeholder="filter" class="border-b outline-none p-3 -mx-3 pt-0"
                            type="text">
                        ...
                        @if ($assets)
                            @foreach ($assets as $asset)
                                <div x-show="$el.innerText.toLowerCase().includes(filter.toLowerCase())"
                                    ...
                                    <input x-model="options" id="{{ $asset->id }}" type="checkbox"
                                        value="{{ $asset->id }}"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                                        wire:model="selected_assets">
                                    <label for="{{ $asset->short_description }}"
            ...
            <x-slot name="right_panel">
                <div x-data="{
                    ...
                    <div class="p-3 rounded-lg flex gap-3 w-full shadow-lg x-50 absolute flex-col bg-white mt-3 z-50"
                        x-show="open && nonuser == false" x-trap="open" @click.outside="open = false"
                        @keydown.escape.window="open = false"
                        x-transition:enter=" ease-[cubic-bezier(.3,2.3,.6,1)] duration-200"
                        x-transition:enter-start="!opacity-0 !mt-0" x-transition:enter-end="!opacity-1 !mt-3"
                        x-transition:leave=" ease-out duration-200" x-transition:leave-start="!opacity-1 !mt-3"
                        x-transition:leave-end="!opacity-0 !mt-0">
                        <input x-model="filter" placeholder="Contact Name"
                            class="border-b outline-none p-3 -mx-3 pt-0" type="text"
                        ...
                        @if ($contacts)
                            @foreach ($contacts as $contact)
                                <div x-show="$el.innerText.toLowerCase().includes(filter.toLowerCase())"
                                    class="flex items-center">
                                    <input x-model="selectedContacts" id="{{ $contact->id }}" type="radio"
                                        value="{{ $contact->id }}"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
