@php
    ...
    $rootDiv =
        'justify-center p-4 items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-white dark:bg-gray-700';
...
<div class="{{ $rootDiv }}" style="{{ $scrollbar }}">
    <div class="mb-4 mt-5">
        ...
        <hr class="border-slate-500 mb-5" />
    </div>
    @if ($canEdit)
        <div class="mb-10">
            <button class="{{ $buttonBlue }}" wire:click='exportLogs'>
                Export Logs (Excel)
            </button>
        </div>
    @endif
            ...
            @foreach ($taskLogs as $taskLog)
                <li class="{{ $liItem }}">
                    ...
                    <time class="{{ $liDatetime }}">
                        {{ date('F jS, Y H:i', strtotime($taskLog->created_at)) }} |
                        Created by {{ $taskLog->user_id }}
                    </time>
                    <p class="{{ $liPContent }}">
                        {{ $taskLog->description }}
                    </p>
                    @if ($taskLog->source == $external)
