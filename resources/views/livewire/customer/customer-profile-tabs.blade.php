    ...
    <x-tab>
        <x-slot name="tabContent">
            @foreach ($tabContents as $tab)
                ...
                <li class="me-2">
                    <a href="#" class="{{ $tab['name'] == $selectedTab ? $activeTab : $inactiveTab }}"
            ...
            @isset($tabMore)
                @if ($selectedTab)
                    @if($selectedTab == 'Assets')
                        <livewire:customer.customer-profile-assets :customer="$customer">  
                    @endif
                    @if($selectedTab == 'Customer Contacts')
                        <livewire:customer.customer-profile-contacts :customer="$customer">  
                    @endif
                @endif
            @endisset
        </x-slot>
